...
\end{example}
To run this in parallel using two \emph{forked} parallel processes, we
do:
...
\end{example}
Finally, to run this in parallel using two R parallel processes
running in the \emph{background}, we do:
...
the top-1.1\% most downloaded packages on CRAN, and there are nearly 800
...
Relaying of standard output, messages, warnings, and errors simplify
...
across the widerange of operating systems (Linux, Solaris, macOS, and
...
transfering them to a job queue that, in turn, distributes them to
