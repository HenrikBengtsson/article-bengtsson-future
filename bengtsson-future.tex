...
with more, although in practice%
\footnote{The \pkg{parallel} package does not export generic functions
\code{sendData()} and \code{recvData()}. If these would be exported, then
it would be possible to implement also other types of ``snow'' cluster
backends.}
...
the \pkg{future} package is tested against 166 direct reverse-package
...
functions \code{serialize()} and \code{unserialize()} via
