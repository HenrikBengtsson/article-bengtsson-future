...
tasks.  As the size of these tasks tend to grow over time, and access
to more CPU cores, so does the demand for parallel-processing
...
via \pkg{parallel} package with version 2.14.0 (2011), which to date
...
the \pkg{parallel} package provides, and those that are used to
...
efficient parallelization technique, which operates at the core of
...
refer to as \dfn{multi-process} parallelization and which is a coarser
...
and \dfn{promises} \citep{FriedmanWise_1978, Hibbard_1976}.  To better
understand how it fits in among and relates to existing
...
forking. In contrast, when using \pkg{doParallel} with SNOW clusters,
globals and packages need to be identified and explicitly exported,
via additional arguments \code{.export} and \code{.packages}
...
\code{slow\_fcn(x)} with \code{x} being \code{1}, then re-assigns
...
\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.7\linewidth]{figures/future-lapply-4workers}
    ...
    used.  Less verbose, map-reduce alternatives are available in
...
\code{foreach()}.  Indeed, we could re-implemented these
...
promise.  As soon as we try to use \code{v}, say,
\begin{example}
y <- sqrt(v) 
\end{example}
...
using \pkg{revdepcheck}~\citep{GitHub:revdepcheck}.  As of March 2021,
the \pkg{future} package is tested against 143 direct reverse-package
...
via \dfn{unserialization}.  This is ability is fundamental to all
...
was \pkg{future.batchtools}' predecessor
\CRANpkg{future.BatchJobs} (legacy, archived), which was straightforward to implement on top of
...
grown steadily ever since.  As of March 2021, there are 136 packages on
...
and \pkg{furrr}, the corresponding number of packages are 70 and 40,
respectively.
...
extra processing time are from spawning new parallel processes,
...
applied on a single large data frame should probably be parallized on
the local computer with forked processing, if supported, rather than
being distributed to a compute cluster running in the cloud.  In
...
\CRANpkg{profvis}~\citep{CRAN:profvis}.  These tools can also be
used to identify different sources of overhead in the parallelization
framework itself, including the ones of the future ecosystem.  It is
on the roadmap to make futures collect and report on some of these
benchmarks automatically to help developers optimize their code and
for end-users to choose a proper backend.
...
of these we could use \dfn{marshaling} to encode before serializing
them such that a working clone can be reconstructed after
unserializing and \dfn{unmarshaling}.  As an example, a read-only
file connection can marshaled by recording its filename and file
...
by base R functions \code{serialize()} and \code{unserialize()} via
argument \code{refhook}, marshaling is rarely used concept in R,
...
developers to register custom \code{marshal()} and \code{unmarshal()}
