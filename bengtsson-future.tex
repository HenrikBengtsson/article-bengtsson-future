...
y <- sqrt(v) 
\end{example}
the associated promise will call \code{value()} on the underlying future, while possibly blocking, and at the end assign the collected result to \code{v}\footnote{The internal call to \code{value()} will also cause any captured standard output and conditions to be relayed.}.
