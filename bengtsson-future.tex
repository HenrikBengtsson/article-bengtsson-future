...
for working with \emph{parallel data structure}, e.g.
\CRANpkg{sparklyr}~\citep{CRAN:sparklyr}, and the
\dfn{Programming with Big Data in R} (pbdR) project~\citep{Schmidt2017}.
By pre-distributing data and storing them on parallel workers, the
overhead from passing data on-the-fly in parallel processing can be
decreased, resulting in an overall faster processing time but also
lower and more fine-tuned memory requirements.
%
