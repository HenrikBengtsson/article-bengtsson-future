...
%
In addition to parallelize computations, there are also efforts in R
for working with \emph{parallel data structure},
e.g. \CRANpkg{sparklyr}~\citep{CRAN:sparklyr}, and
the \dfn{Programming with Big Data in R} (pbdR)
project~\citep{project:pbdR}.  By pre-distributing data and storing
them on parallel workers, the overhead from passing data on-the-fly in
parallel processing can be decreased, resulting in an overall faster
processing time but also lower and more fine-tuned memory
requirements.
%
...
clusters (from their origin in the \pkg{snow} package), e.g.
...
first released supports this. As of April 2021, \pkg{foreach} is among
...
\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.75\linewidth]{figures/future-lapply-4workers}
...
using \pkg{revdepcheck}~\citep{GitHub:revdepcheck}.  As of April 2021,
the \pkg{future} package is tested against 167 direct reverse-package
...
grown steadily ever since.  As of April 2021, \pkg{future} is among
...
map-reduce parallelization packages \pkg{future.apply} (top-2.5\% most
downloaded) and \pkg{furrr} (top-1.6\%), the corresponding number of
...
example, the workflow package \CRANpkg{drake}~\citep{Landau_2018}
...
Another prominent example is the \CRANpkg{shiny}
package~\citep{CRAN:shiny}, which implements support
for \dfn{asynchronous processing} in Shiny application via futures.
Asynchronous processing helps to prevent long-running tasks from
blocking the user interface.
Similarly, the \CRANpkg{plumber} package~\citep{CRAN:plumber}, which
