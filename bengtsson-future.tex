...
parallelization solutions in R, let us revisit the two most well-known
solutions - packages \pkg{parallel} and \CRANpkg{foreach} - below.
For an overview of parallel techniques available to R developers,
including the ones based on native code\footnote{Although the concept
of futures could also apply to C, C++, and Fortran parallelization,
the future framework targets parallelization at the R level and does
not provide an implementation for native code.} not discussed here,
see \citet{Eddelbuettel2020} and the
...
of these we could use \dfn{marshalling} to encode before serializing
them such that a working clone can be reconstructed after
unserializing and \dfn{unmarshalling}.  As an example, a read-only
...
argument \code{refhook}, marshalling is rarely used concept in R,
