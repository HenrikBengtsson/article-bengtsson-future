...
facilitate most types of parallel processing in R.  It builds upon the
concepts of \dfn{futures} \citep{HewittBaker_1977} and
\dfn{promises} \citep{FriedmanWise_1978, Hibbard_1976}.
...
solutions - packages \pkg{parallel} and \CRANpkg{foreach}.
...
and \code{parLapply()}, which mimic the behavior of the map-reduce
function \code{lapply()} in the \pkg{base} package.  Here is an
example showing them calling a ``slow'' function on each
element in a vector using two parallel workers:
%
\begin{example}
xs <- 1:10
...
e.g.\ \CRANpkg{doMPI}~\citep{CRAN:doRedis} and
...
are produced when not using futures.  For example, just like we get:
\begin{example}
x <- "24"
v <- log(x)
...
# Error in log(x) : non-numeric argument to mathematical function
\end{example}
with futures.  Standard mechanisms for condition handling apply to
these relayed errors.  For example, to assign a missing value
...
core to make it as easy as possible to produce statistical sound and
...
since it is computational cheap to detect when a future expression
...
environment is technically an \dfn{environment} that emulates most of
the properties that a \emph{list} have, including indexing as in:
\begin{example}
xs <- 1:10
vs <- listenv::listenv()
for (i in seq_along(xs)) {
...
Shiny application via futures.  Asynchronous processing help
...
use \code{value()} to collect its value.  Since neither has been
...
reports, and interesting and fruitful discussions.  The development of
